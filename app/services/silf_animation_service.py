...
_COMMON = """- Output ONLY raw HTML starting with <!DOCTYPE html>. No markdown fences, no commentary.
...
_ANIM_SYSTEM = f"""You are a Principal Frontend Engineer and motion-graphics educator. Generate ONE complete, self-contained, browser-ready HTML file: a SHORT LOOPING ANIMATED DIAGRAM showing a single scientific mechanism happening, for a school student.
...
_PANEL_SYSTEM = f"""You are a Principal Frontend Engineer and editorial illustrator. Generate ONE complete, self-contained, browser-ready HTML file: a SINGLE CLEAN VECTOR SCENE PANEL illustrating one story moment for a school student (Brilliant/Khan editorial style, NOT a detailed cartoon).
...
async def generate_silf_animation(
    *, topic: str, subject: str, grade: int, animation_brief: str, summary_text: str = "",
    ...
    html, usage = await _generate_visual(_ANIM_SYSTEM, msg, [settings.GEMINI_SIMULATION_MODEL, settings.GEMINI_SIMULATION_MODEL], "SILF_ANIM")
    if html:
...
async def generate_scene_panel(
    *, topic: str, subject: str, grade: int, panel_brief: str, summary_text: str = "",
    ...
    html, usage = await _generate_visual(
        _PANEL_SYSTEM, msg, [settings.GEMINI_STORY_MODEL, settings.GEMINI_SIMULATION_MODEL], "SILF_PANEL"
    )
    if html:
